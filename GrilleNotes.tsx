        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 600 }}>
          <thead>
            <tr style={{ background: '#f9fafb' }}>
              <th scope="col" style={{ padding: '8px 12px', textAlign: 'left', fontSize: 11, color: '#6b7280', borderBottom: '2px solid #e5e7eb', position: 'sticky', left: 0, background: '#f9fafb', zIndex: 1, minWidth: 180 }}>
                Étudiant
              </th>
              {evalsCC.map(e => (
                <th scope="col" key={e.id} style={{ padding: '8px 10px', textAlign: 'center', fontSize: 10, color: '#1d4ed8', borderBottom: '2px solid #dbeafe', minWidth: 90 }}>
              ...
              {evalsEX.map(e => (
                <th scope="col" key={e.id} style={{ padding: '8px 10px', textAlign: 'center', fontSize: 10, color: '#7c3aed', borderBottom: '2px solid #ede9fe', minWidth: 90 }}>
                  <div style={{ fontWeight: 700 }}>EXAMEN</div>
                  <div style={{ fontWeight: 400, color: '#6b7280', fontSize: 9 }}>{e.intitule || e.format}</div>
                </th>
              ))}
              <th scope="col" style={{ padding: '8px 10px', textAlign: 'center', fontSize: 10, color: '#059669', borderBottom: '2px solid #dcfce7', minWidth: 70 }}>Moy. CC</th>
              <th scope="col" style={{ padding: '8px 10px', textAlign: 'center', fontSize: 10, color: '#374151', borderBottom: '2px solid #e5e7eb', minWidth: 80 }}>Note finale</th>
            </tr>
          </thead>
          <tbody>
                      ...
                      <td key={e.id} style={{ padding: '4px 6px', textAlign: 'center', background: bg }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 2, justifyContent: 'center' }}>
                          <input
                            ...
                            placeholder="—"
                            aria-label={`Note CC ${e.intitule || e.format} — ${et.nom} ${et.prenom ?? ''}`}
                            aria-invalid={isErr}
                          ...
                          />
                          {isErr && (
                            <span id={`err-${key}`} role="alert" style={srOnly}>
                              Note invalide pour {et.nom} {et.prenom ?? ''}, {e.intitule || e.format} : doit être comprise entre 0 et 20.
                            </span>
                          )}
                          <button
                            disabled={sessLocked}
                            ...
                            aria-pressed={isAbs}
                            aria-label={`${isAbs ? 'Marquer présent' : 'Marquer absent'} : ${et.nom} ${et.prenom ?? ''}, ${e.intitule || e.format}`}
                      ...
                      <td key={e.id} style={{ padding: '4px 6px', textAlign: 'center', background: bg }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 2, justifyContent: 'center' }}>
                          <input
                            ...
                            disabled={isAbs || sessLocked || isSav}
                            placeholder="—"
                            aria-label={`Note Examen ${e.intitule || e.format} — ${et.nom} ${et.prenom ?? ''}`}
                            aria-invalid={isErr}
                            aria-describedby={isErr ? `err-${key}` : undefined}
                            title={isSav ? 'Sauvegarde…' : isErr ? errors[key] : ''}
                            style={{
