...
Portail Public et Portail Enseignant n'utilisent **aucune classe CSS partagée** — tout le styling passe par un objet `S = { ... }` local à chaque fichier (boutons, champs, tableau, tabs, empty state, toggle, modal, tout y est redéfini indépendamment). Résultat visuellement cohérent avec le reste de l'app (mêmes rayons, mêmes familles de couleurs), mais entièrement dupliqué entre les deux fichiers plutôt que partagé — et porteur de la variante `#1e3a5f` mentionnée en section 1. C'est le pattern dominant des pages "portail" (public + enseignant), à l'inverse des pages de gestion (Délibérations, Enseignants, Messages) qui mélangent classes partagées et styles inline ponctuels.

### 4.16 Carte de profil (dégradé)

Pattern unique à Portail Enseignant : bandeau d'en-tête en `linear-gradient(135deg, #1e3a5f, #2563eb)`, radius 16px, avatar à initiales sur fond blanc semi-transparent (`rgba(255,255,255,.2)`), texte blanc avec opacité dégressive pour la hiérarchie (title 100%, sous-titre 85%, meta 70%). Seule occurrence de dégradé dans l'échantillon audité — et seule occurrence de `#2563eb` (bleu vif, hors palette de marque).

### 4.17 Carte sélectionnable

Vu dans Portail Enseignant (sélection de matière) : état non sélectionné = bordure fine `#f1f5f9` + ombre légère ; état sélectionné = bordure 2px `#1e3a5f` + ombre portée colorée (`0 4px 12px rgba(30,58,95,.15)`). Toute la carte est cliquable (`cursor: pointer`), transition `.15s` sur tous les attributs visuels.

### 4.18 Bloc statistique (stat row)

Distinct des cartes KPI de 4.9 : rangée flex de cartes simples (`statCard`), chacune avec un grand nombre (22px/700, couleur `#1e3a5f` par défaut ou codée par sémantique — vert admis, rouge absences) et un label 11px gris en dessous. Plus sobre que les cartes KPI à icône — à réserver aux sous-vues (notes d'une matière, séances) plutôt qu'aux vues de synthèse générale.

### 4.19 Tableau éditable avec suivi des modifications

Pattern de saisie de notes : les lignes modifiées mais non sauvegardées sont surlignées (`background: #fffbeb`), un badge ambre récapitule le nombre de modifications en attente (`"3 modification(s) non sauvegardée(s)"`), le bouton d'enregistrement reste désactivé tant qu'aucune modification n'est détectée. Les valeurs calculées (moyenne finale) se recalculent en direct à partir des `edits` locaux avant sauvegarde.

### 4.20 Groupe de boutons de sélection de statut

Alternative aux boutons radio natifs, vue dans le formulaire de présence : rangée de petits boutons (`present` / `absent` / `justifie`), l'état actif prend la couleur sémantique correspondante (vert/rouge/jaune) en fond + bordure, les autres restent neutres (`btnSmall` par défaut). Complète le pattern toggle de 4.12 pour les choix à 3+ options.

### 4.21 Grille d'emploi du temps

Colonnes = jours de la semaine (uniquement les jours ayant des créneaux), chaque colonne a un en-tête plein navy et une liste de créneaux colorés cycliquement à partir d'une palette pastel fixe (`COLORS`, 7 teintes). Pattern isolé, non réutilisé ailleurs dans l'échantillon.

### 4.22 Variante de structure modale

Portail Enseignant introduit une **troisième** structure de modal (après `.modal-overlay`/`.modal` du CSS partagé et le modal ad-hoc de Portail Public) : objets dédiés `S.overlay` / `S.modal` / `S.modalHeader` / `S.modalBody` / `S.modalFooter`, avec `modalBody` en `overflowY: auto` et `flexDirection: column` pour les contenus longs (liste d'étudiants scrollable). Structurellement plus propre que les deux précédentes (séparation claire header/body/footer) mais toujours non partagée entre pages.
...
- **Navy à deux valeurs** : `#1B2A4A` (canonique, CSS partagé) vs `#1e3a5f` (Délibérations, Portail Public, Portail Enseignant, Messages). C'est en réalité la valeur la **plus répandue** dans les pages métier échantillonnées — à considérer sérieusement comme candidate à la promotion en couleur canonique plutôt que comme une simple dérive. Cf. section 1.
- **Bleu vif isolé `#2563eb`** : utilisé uniquement dans le dégradé de la carte de profil du Portail Enseignant (`linear-gradient(135deg, #1e3a5f, #2563eb)`). Hors palette de marque — à valider si c'est un choix volontaire pour distinguer visuellement les portails "self-service" des pages de gestion.
...
- **Trois structures de modal différentes** : `.modal-overlay`/`.modal` (CSS partagé), modal ad-hoc de Portail Public, et structure `overlay`/`modal`/`modalHeader`/`modalBody`/`modalFooter` du Portail Enseignant (la plus aboutie des trois). Bon candidat à l'unification en composant `<Modal>` partagé.
- **Toggle personnalisé sans sémantique ARIA** (`role="switch"`, `aria-checked`) — cf. 4.12.
- **Duplication de styles** : les deux pages "portail" (Public et Enseignant) réimplémentent chacune l'intégralité du système visuel via un objet `S` local plutôt que les classes partagées — cf. 4.15.
- **Deux échelles de mention distinctes potentielles** : Portail Enseignant calcule ses propres mentions (TB/B/AB/P/F, seuils 16/14/12/10) localement dans `getMention()`, séparément du système `MENTION_LABEL`/`MENTION_COLOR` centralisé utilisé par le module Délibérations. À vérifier que les deux logiques restent alignées si les seuils venaient à changer.
...
- Modules non couverts par cet audit : formulaires de programmes/UE, saisie de notes côté administration (module `saisie-notes` distinct de la vue enseignant), résultats, relevés, comptabilité, RH, prospects, monitoring, audit, paramètres.
