...
//
// B15 action 2 — charte email : les contenus « text » et les fragments HTML
// sont désormais habillés du gabarit navy/ocre partagé (_shared/email-charte.ts),
// avec le nom/logo de l'école de l'appelant en en-tête. Un document HTML
// complet fourni par l'appelant (contenant <html>) reste envoyé tel quel.
//
...
serve(async (req) => {
  ...
  try {
    ...
    // ── 6bis. B15 action 2 — habillage charté du contenu
    // Branding : nom/logo de l'école de l'appelant (wordmark générique pour
    // le superadmin réseau, dont ecole_id est NULL).
    let branding: { nom: string; logo_url: string | null } = { nom: "EduLink Sup", logo_url: null };
    if (profil.ecole_id) {
      const { data: ecoleBrand } = await supabase
        .from("ecoles").select("nom, logo_url").eq("id", profil.ecole_id).single();
      if (ecoleBrand) branding = ecoleBrand;
    }
    let htmlContent: string;
    if (body.html && /<html[\s>]/i.test(body.html)) {
      // Document HTML complet fourni par l'appelant : envoyé tel quel.
      htmlContent = body.html;
    } else {
      const corpsHtml = body.html
        ? body.html
        : escapeHtml(body.text || "").replace(/\n/g, "<br/>");
      htmlContent = buildEmailCharteHtml({
        sujet: body.subject,
        corpsHtml,
        ecole: branding,
        salutationHtml: body.toName ? `Bonjour <strong>${escapeHtml(body.toName)}</strong>,` : null,
        piedNotes: [
          `Message envoyé via EduLink Sup pour ${escapeHtml(branding.nom)}. Merci de ne pas répondre directement à cet email.`,
        ],
      });
    }
    // ── 7. Appel à Brevo
