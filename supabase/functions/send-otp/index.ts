...
//
// B15 action 2 : l'email OTP adopte le gabarit charté complet
// (_shared/email-charte.ts) — l'encadré de code navy/crème/ocre est conservé.
//
...
import { buildEmailCharteHtml, EMAIL_CHARTE } from "../_shared/email-charte.ts";
const corsHeaders = {
...
async function envoiEmailOTP(apiKey: string, fromEmail: string, fromName: string, to: string, code: string) {
  ...
  const corpsHtml = `
    <p style="margin:0 0 4px;">Voici votre code de connexion EduLink Sup :</p>
    <div style="background:${C.cream};border:1px solid ${C.ocre};border-radius:8px;padding:16px;text-align:center;margin:16px 0;">
      <span style="font-size:28px;font-weight:700;letter-spacing:6px;color:${C.navy};">${code}</span>
    </div>
    <p style="margin:0;color:#6b7280;font-size:12px;">Ce code expire dans ${OTP_VALIDITE_MINUTES} minutes. Ne le partagez avec personne.</p>`;
  const htmlContent = buildEmailCharteHtml({
    sujet: "Votre code de connexion EduLink Sup",
    corpsHtml,
    badge: "Sécurité",
    piedNotes: [
      "Si vous n'êtes pas à l'origine de cette demande, ignorez cet email et signalez-le à votre administrateur.",
    ],
  });
