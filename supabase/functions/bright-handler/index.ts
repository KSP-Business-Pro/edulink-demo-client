...
function checkRateLimit(phone) {
  ...
  if (!entry || now > entry.resetAt) {
    ...
    return {
      allowed: true,
      remaining: RATE_LIMIT_PER_PHONE - 1
    };
  }
  if (entry.count >= RATE_LIMIT_PER_PHONE) return {
    allowed: false,
    remaining: 0
  };
  entry.count++;
  return {
    allowed: true,
    remaining: RATE_LIMIT_PER_PHONE - entry.count
  };
}
...
// ═══════════════════════════════════════════════════════
async function envoiSMS(telephone, code) {
  if (MODE_DEMO) {
  ...
  const content = `EduLink : votre code de connexion est ${code}. ` + `Valable ${OTP_VALIDITE_MINUTES} min. Ne le partagez pas.`;
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      if (resp.status === 401) return {
        ok: false,
        error: "Identifiants Twilio invalides"
      };
      if (data.code === 21211) return {
        ok: false,
        error: "Numéro destinataire invalide"
      };
      if (data.code === 21408 || data.code === 21610) return {
        ok: false,
        error: "Numéro non autorisé"
      };
      return {
        ok: false,
        error: `Erreur Twilio ${data.code || resp.status}: ${data.message}`
      };
    }
...
async function signInOuCreerEtudiant(supabase, etudiant, emailAuth, passwordAuth) {
  ...
  if (createError && String(createError.message || "").toLowerCase().includes("already")) {
    ...
    if (retryError || !retryData?.session) {
      ...
      return {
        error: "Compte existant non synchronisable. Contactez l'administrateur."
      };
    }
    // Lier l'auth_id dans etudiants
    await supabase.from("etudiants").update({
      auth_id: retryData.user.id,
  ...
  if (createError || !createData?.user) {
    ...
    return {
      error: createError?.message || "Échec création compte Auth"
    };
  }
  // Lier l'auth_id dans la table etudiants
  const { error: linkError } = await supabase.from("etudiants").update({
    auth_id: createData.user.id,
    email_auth: emailAuth
  }).eq("id", etudiant.id);
  if (linkError) {
  ...
  if (finalError || !finalSignIn?.session) {
    ...
    return {
      error: "Échec authentification après création"
    };
  }
...
// ═══════════════════════════════════════════════════════
async function signInOuCreerEtudiantV7(supabase, etudiant, emailAuth, securePassword, matricule) {
  ...
  if (!etudiant.auth_id) {
    ...
    if (createError) {
      if (String(createError.message || "").toLowerCase().includes("already")) {
        ...
        const existing = ul?.users?.find((u)=>u.email === emailAuth);
        if (existing) {
          ...
          if (si?.session) return {
            session: si.session,
            user: si.user,
            mode: "orphan-reset-v7"
          };
        }
        return {
          error: "Compte existant non recuperable."
        };
      }
      return {
        error: createError.message || "Echec creation compte"
      };
    }
    await supabase.from("etudiants").update({
      auth_id: createData.user.id,
    ...
    if (!si?.session) return {
      error: "Echec sign-in apres creation"
    };
    return {
      session: si.session,
      user: si.user,
  ...
  if (passwordSetEnBase) {
    const { error: updErr } = await supabase.auth.admin.updateUserById(etudiant.auth_id, {
      password: securePassword
    });
    if (updErr) return {
      error: "Erreur rotation password."
    };
    const { data: si, error: siErr } = await supabase.auth.signInWithPassword({
      email: emailAuth,
      password: securePassword
    });
    if (siErr || !si?.session) return {
  ...
  const { data: siOld } = await supabase.auth.signInWithPassword({
    email: emailAuth,
    password: matricule
  });
  if (!siOld?.session) {
    // Matricule invalide : reset force
    await supabase.auth.admin.updateUserById(etudiant.auth_id, {
      password: securePassword
    });
    await supabase.from("etudiants").update({
      auth_password_set: true
    }).eq("id", etudiant.id);
    const { data: siNew } = await supabase.auth.signInWithPassword({
      email: emailAuth,
      password: securePassword
    });
    if (!siNew?.session) return {
      error: "Echec apres reset force."
    };
    return {
      session: siNew.session,
      user: siNew.user,
      mode: "forced-reset-v7"
    };
  }
...
serve(async (req)=>{
  ...
  try {
    ...
    if (action === "send") {
      ...
      if (insertError) {
        return new Response(JSON.stringify({
          error: "Erreur stockage OTP : " + insertError.message
        }), {
          status: 500,
          headers: {
            ...corsHeaders,
    ...
    if (action === "verify") {
      ...
      const { data: otp, error: selectError } = await supabase.from("otp_codes").select("*").eq("telephone", telephone).eq("valide", false).order("created_at", {
        ascending: false
      }).limit(1).maybeSingle();
      if (selectError) {
        return new Response(JSON.stringify({
      ...
      const authExpireAt = new Date(Date.now() + AUTH_TOKEN_VALIDITE_MINUTES * 60_000).toISOString();
      await supabase.from("auth_tokens").update({
        consomme: true,
        consomme_at: new Date().toISOString()
      }).eq("telephone", telephone).eq("consomme", false);
      // v6 : INSERT avec vérification stricte — ne pas retourner le token si l'INSERT échoue
      const { data: insertedToken, error: tokenError } = await supabase.from("auth_tokens").insert({
        telephone,
        token: authToken,
      ...
      if (tokenError || !insertedToken) {
        ...
        return new Response(JSON.stringify({
          error: "Erreur création token d'authentification. Réessayez."
        }), {
          status: 500,
          headers: {
            ...corsHeaders,
    ...
    if (action === "exchange") {
      ...
      const { data: tokenRow, error: tokenSelectError } = await supabase.from("auth_tokens").select("*").eq("token", authToken).eq("consomme", false).maybeSingle();
      if (tokenSelectError) {
        return new Response(JSON.stringify({
          error: "Erreur serveur"
        }), {
      ...
      if (new Date(tokenRow.expire_at) < new Date()) {
        return new Response(JSON.stringify({
          error: "Token expiré. Recommencez la connexion."
        }), {
          status: 410,
          headers: {
            ...corsHeaders,
      ...
      const { data: etudiantsList, error: etudiantError } = await supabase.from("etudiants").select("id, matricule, email_parent, email_auth, auth_id, ecole_id, auth_password_set").eq("telephone_parent", telephone).eq("is_demo", false).order("created_at", {
        ascending: false
      });
      if (etudiantError || !etudiantsList || etudiantsList.length === 0) {
        ...
        return new Response(JSON.stringify({
          error: "Étudiant non trouvé pour ce numéro."
        }), {
          status: 404,
          headers: {
            ...corsHeaders,
      ...
      const securePassword = Array.from(crypto.getRandomValues(new Uint8Array(32))).map((b)=>b.toString(16).padStart(2, "0")).join("");
      // ── v5/v7 : sign-in OU création à la volée
      const result = await signInOuCreerEtudiantV7(supabase, etudiant, emailAuth, securePassword, matricule);
      if (result.error || !result.session) {
        console.error("[bright-handler v5] Échec final exchange:", result.error);
        return new Response(JSON.stringify({
          error: "Échec d'authentification. Contactez l'administrateur."
        }), {
          status: 401,
          headers: {
            ...corsHeaders,
      ...
      console.log(`[bright-handler v5] ✅ Exchange réussi pour ${telephone} → ${emailAuth} (mode=${result.mode})`);
      return new Response(JSON.stringify({
        success: true,
    ...
    return new Response(JSON.stringify({
      error: "Action inconnue. Utilisez 'send', 'verify' ou 'exchange'."
    }), {
      status: 400,
      headers: {
        ...corsHeaders,
