      ...
      <div className="modal" style={{ width: 520, padding: '1.5rem' }}>
        ...
        <form onSubmit={handleSubmit} autoComplete="off">
          <div style={{ marginBottom: '.85rem' }}>
            <label htmlFor="fact-etudiant">Étudiant *</label>
            <select id="fact-etudiant" name="etudiant" value={etudiantId} onChange={e => setEtudiantId(e.target.value)} style={{ width: '100%', marginTop: 4 }} required aria-describedby={error ? 'fact-error' : undefined}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.75rem', marginBottom: '.85rem' }}>
            <div>
              <label htmlFor="fact-type">Type de frais *</label>
              <select id="fact-type" name="typeFrais" value={typeFrais} onChange={e => { setTypeFrais(e.target.value as TypeFrais); setLibelleManual(false); }} style={{ width: '100%', marginTop: 4 }} required>
                {TYPES.map(t => <option key={t} value={t}>{RUBRIQUE_LABELS[t]}</option>)}
              </select>
            </div>
            <div>
              <label htmlFor="fact-annee">Année scolaire</label>
              <input type="text" id="fact-annee" name="annee" value={annee} onChange={e => { setAnnee(e.target.value); setLibelleManual(false); }} style={{ width: '100%', marginTop: 4 }} placeholder="2025-2026" />
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '.75rem', marginBottom: '.85rem' }}>
            <div>
              <label htmlFor="fact-libelle">Libellé *</label>
              <input type="text" id="fact-libelle" name="libelle" value={libelle} onChange={e => { setLibelle(e.target.value); setLibelleManual(true); }} style={{ width: '100%', marginTop: 4 }} required />
            </div>
            <div>
              <label htmlFor="fact-montant">Montant (FCFA) *</label>
              <input type="number" id="fact-montant" name="montant" value={montant} onChange={e => setMontant(e.target.value)} style={{ width: '100%', marginTop: 4 }} min={1} step="any" required aria-describedby={error ? 'fact-error' : undefined} />
            </div>
          </div>
          <div style={{ marginBottom: '1.2rem' }}>
            <label htmlFor="fact-echeance">Date d'échéance</label>
            <input type="date" id="fact-echeance" name="echeance" value={echeance} onChange={e => setEcheance(e.target.value)} style={{ width: '100%', marginTop: 4 }} />
          </div>
          {error && <div id="fact-error" role="alert" style={{ background: '#fee2e2', color: '#dc2626', padding: '8px 12px', borderRadius: 8, fontSize: 12, marginBottom: '1rem' }}>{error}</div>}
  ...
  return (
    <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ width: 520, padding: '1.5rem' }}>
        ...
        <form onSubmit={handleSubmit} autoComplete="off">
          <div style={{ marginBottom: '.85rem' }}>
            <label htmlFor="fm-semestre">Semestre / Promotion *</label>
            <select id="fm-semestre" name="semestre" value={semId} onChange={e => setSemId(e.target.value)} style={{ width: '100%', marginTop: 4 }} required aria-describedby={error ? 'fm-error' : undefined}>
              <option value="">— Sélectionner —</option>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.75rem', marginBottom: '.85rem' }}>
            <div>
              <label htmlFor="fm-type">Type de frais *</label>
              <select id="fm-type" name="typeFrais" value={typeFrais} onChange={e => { setTypeFrais(e.target.value as TypeFrais); setLibelleManual(false); }} style={{ width: '100%', marginTop: 4 }} required>
                {TYPES.map(t => <option key={t} value={t}>{RUBRIQUE_LABELS[t]}</option>)}
              </select>
            </div>
            <div>
              <label htmlFor="fm-annee">Année scolaire *</label>
              <input type="text" id="fm-annee" name="annee" value={annee} onChange={e => { setAnnee(e.target.value); setLibelleManual(false); }} style={{ width: '100%', marginTop: 4 }} required />
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: '.75rem', marginBottom: '.85rem' }}>
            <div>
              <label htmlFor="fm-libelle">Libellé *</label>
              <input type="text" id="fm-libelle" name="libelle" value={libelle} onChange={e => { setLibelle(e.target.value); setLibelleManual(true); }} style={{ width: '100%', marginTop: 4 }} required />
            </div>
            <div>
              <label htmlFor="fm-montant">Montant / étudiant (FCFA) *</label>
              <input type="number" id="fm-montant" name="montant" value={montant} onChange={e => setMontant(e.target.value)} style={{ width: '100%', marginTop: 4 }} min={1} step="any" required aria-describedby={error ? 'fm-error' : undefined} />
            </div>
          </div>
          <div style={{ marginBottom: '1.2rem' }}>
            <label htmlFor="fm-echeance">Date d'échéance</label>
            <input type="date" id="fm-echeance" name="echeance" value={echeance} onChange={e => setEcheance(e.target.value)} style={{ width: '100%', marginTop: 4 }} />
          </div>
          {error && <div id="fm-error" role="alert" style={{ background: '#fee2e2', color: '#dc2626', padding: '8px 12px', borderRadius: 8, fontSize: 12, marginBottom: '1rem' }}>{error}</div>}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '.5rem', paddingTop: '.85rem', borderTop: '1px solid #f3f4f6' }}>
            <button type="button" className="btn-ghost" onClick={onClose}>Annuler</button>
