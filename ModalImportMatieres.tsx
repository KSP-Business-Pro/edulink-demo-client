    ...
    <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal" style={{ width: 580, padding: '1.5rem' }}>
        ...
        <div style={{ marginBottom: '.85rem' }}>
          <label htmlFor="import-mat-fichier">Fichier CSV ou Excel *</label>
          <input ref={fileRef} id="import-mat-fichier" name="fichier" type="file" accept=".csv,.xlsx,.xls" onChange={handleFile}
            aria-describedby={parseErr ? 'import-mat-error' : undefined}
            style={{ width: '100%', marginTop: 4 }} />
        </div>

        {parseErr && (
          <div id="import-mat-error" role="alert" style={{ background: '#fee2e2', color: '#dc2626', padding: '8px 12px', borderRadius: 8, fontSize: 12, marginBottom: '.85rem' }}>
            {parseErr}
          </div>
        )}
