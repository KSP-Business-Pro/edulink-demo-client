    ...
    <div style={S.overlay} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={S.modal}>
        ...
        <div style={S.body}>
          {/* Semestre */}
          <div style={S.field}>
            <label htmlFor="insc-indiv-semestre" style={S.label}>Semestre <span style={{ color: '#dc2626' }}>*</span></label>
            <select
              id="insc-indiv-semestre"
              name="semestre"
              style={S.select}
              value={semestreId}
              ...
              aria-describedby={error ? 'insc-indiv-error' : undefined}
            >
          ...
          {/* Promotion */}
          <div style={S.field}>
            <label htmlFor="insc-indiv-promotion" style={S.label}>Promotion <span style={{ color: '#dc2626' }}>*</span></label>
            <select
              id="insc-indiv-promotion"
              name="promotion"
              style={S.select}
              value={promotionId}
          ...
          {/* Étudiant */}
          <div style={S.field}>
            <label htmlFor="insc-indiv-etudiant" style={S.label}>
            ...
            {loadingEtu ? (
              <div role="status" style={{ padding: '8px 10px', fontSize: 12, color: '#94a3b8' }}>Chargement…</div>
            ) : (
              <select
                id="insc-indiv-etudiant"
                name="etudiant"
          ...
          <div style={{ ...S.field, marginTop: 4 }}>
            <label htmlFor="insc-indiv-genfacture" style={{ ...S.label, display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
              <input
                type="checkbox"
                id="insc-indiv-genfacture"
                name="genererFacture"
                checked={genFacture}
          ...
          {genFacture && (
            <div style={S.field}>
              <label htmlFor="insc-indiv-montant" style={S.label}>Montant (FCFA) <span style={{ color: '#dc2626' }}>*</span></label>
              <input
                type="number"
                id="insc-indiv-montant"
                name="montant"
                style={S.input}
                value={montant}
          ...
          {/* Messages */}
          {error && (
            <div id="insc-indiv-error" role="alert" style={{ background: '#fef2f2', color: '#991b1b', padding: '8px 12px', borderRadius: 8, fontSize: 12, border: '1px solid #fecaca' }}>
              {error}
            </div>
          )}
          {success && (
            <div role="status" style={{ background: '#f0fdf4', color: '#166534', padding: '8px 12px', borderRadius: 8, fontSize: 12, border: '1px solid #bbf7d0' }}>
              {success}
            </div>
          )}
        </div>
