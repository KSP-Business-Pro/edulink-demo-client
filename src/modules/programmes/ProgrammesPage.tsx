...
export default function ProgrammesPage() {
  ...
  const loadAll = useCallback(async () => {
    ...
    try {
      ...
      // Enseignants (pour ModalMatieres)
  ...
  // ── Groupement programmes par grade ────────────────────────────────────────
    ...
    <div>
      ...
      <div className="top">
        ...
        <div className="top-actions">
          ...
          {tab === 'programmes' && (
            <button className="btn-blue" onClick={() => setModalProg({ open: true, item: null })}>
              + Programme
            </button>
          )}
          {tab === 'ues' && (
            <button className="btn-blue" onClick={() => setModalUE({ open: true, item: null })}>
              + Unité d'Enseignement
            </button>
          )}
          {tab === 'semestres' && (
            <button className="btn-blue" onClick={() => setModalSem({ open: true, item: null })}>
              + Semestre
            </button>
          )}
        </div>
                      ...
                      <tbody>
                        {items.map(p => (
                          <tr key={p.id}>
                            ...
                            <td>
                              <span className={`badge ${p.actif ? 'green' : 'gray'}`}>
                      ...
                      <tbody>
                        {ues.map(u => (
                          <tr key={u.id}>
                            ...
                            <td>
                              <span className={`badge ${typeUEColor[u.type_ue] ?? 'gray'}`}>
                                {u.type_ue}
                              </span>
                            </td>
                            <td><span className="badge teal">{u.credits_cect} CECT</span></td>
                            <td style={{ fontSize: 12 }}>
                              CC {Math.round(u.poids_cc * 100)}% · Exam {Math.round(u.poids_examen * 100)}%
                            </td>
                            <td style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                              <button
                                className="btn-ghost btn-sm"
                                onClick={() => setModalMat({ open: true, ueId: u.id, ueNom: u.intitule })}
                              >
                                Matières
                              </button>
                      ...
                      {semestres.map(s => (
                        <tr key={s.id}>
                          ...
                          <td style={{ fontSize: 12, color: '#6b7280' }}>
                            {s.programmes_lmd?.intitule ?? <span style={{ color: '#d1d5db' }}>—</span>}
                          </td>
                          <td>
                            <span className="badge blue">{s.niveau}</span>
                          </td>
                          <td style={{ fontSize: 12, color: '#6b7280' }}>
                            {s.annees_academiques?.libelle ?? '—'}
                          </td>
                          <td>
      ...
      {modalProg.open && (
        <ModalProgramme
          ecoleId={ecoleId}
          programme={modalProg.item}
          onClose={() => setModalProg({ open: false, item: null })}
          onSaved={loadAll}
        />
      )}
      ...
      {modalUE.open && (
        <ModalUE
          ...
          onClose={() => setModalUE({ open: false, item: null })}
          onSaved={loadAll}
        />
      )}
