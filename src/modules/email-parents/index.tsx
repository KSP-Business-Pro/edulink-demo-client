  ...
  return (
    <div style={S.layout}>
      ...
      <div style={S.leftPane}>
        <div style={S.card}>
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 10 }}>
            ...
            <div role="alert" style={{ fontSize: 12, background: '#fef2f2', color: '#991b1b', padding: '3px 8px', borderRadius: 5 }}>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
            <label htmlFor="emailp-filter-statut" style={SR_ONLY}>Filtrer par statut</label>
            <select id="emailp-filter-statut" name="filterStatut" value={filterStat} onChange={e => setFilterStat(e.target.value)} style={{ ...S.filterSel, flex: 1 }}>
              <option value="">Tous statuts</option>
              <option value="actif">Actifs</option>
              <option value="inactif">Inactifs</option>
            </select>
            <label htmlFor="emailp-filter-promo" style={SR_ONLY}>Filtrer par promotion</label>
            <select id="emailp-filter-promo" name="filterPromotion" value={filterProm} onChange={e => setFilterProm(e.target.value)} style={{ ...S.filterSel, flex: 1 }}>
      ...
      <div style={S.rightPane}>
        ...
        {/* Modèle */}
        <div style={S.field}>
          <label htmlFor="emailp-utiliser-un-mod-le" style={S.label}>Utiliser un modèle</label>
          <select id="emailp-utiliser-un-mod-le" name="emailp-utiliser-un-mod-le" onChange={e => e.target.value && applyModele(e.target.value)} style={S.select} defaultValue="">
        ...
        {/* Catégorie */}
        <div style={S.field}>
          <span id="emailp-categorie-label" style={S.label}>Catégorie</span>
          <div role="group" aria-labelledby="emailp-categorie-label" style={{ display: 'flex', gap: 6, flexWrap: 'wrap' as const }}>
            {CATEGORIES.map(c => (
              <button key={c.id} onClick={() => setCategorie(c.id)} aria-pressed={categorie === c.id}
        ...
        {/* Objet */}
        <div style={S.field}>
          <label htmlFor="emailp-objet" style={S.label}>Objet *</label>
          <input id="emailp-objet" name="emailp-objet" value={objet} onChange={e => setObjet(e.target.value)}
        ...
        <div style={S.field}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <label htmlFor="emailp-corps" style={S.label}>Message *</label>
            <span style={{ fontSize: 11, color: '#94a3b8' }}>{corps.length} caractères</span>
          </div>
          <textarea id="emailp-corps" name="corps" value={corps} onChange={e => setCorps(e.target.value)}
      ...
      {showForm && (
        <div style={{ ...S.card, marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: 8 }}>
            <div style={S.field}>
              <label htmlFor="emailp-nom-du-mod-le" style={S.label}>Nom du modèle</label>
              <input id="emailp-nom-du-mod-le" name="emailp-nom-du-mod-le" value={form.nom} onChange={e => setForm(f => ({ ...f, nom: e.target.value }))} style={S.input} placeholder="Nom du modèle..." />
            </div>
            <div style={S.field}>
              <label htmlFor="emailp-cat-gorie" style={S.label}>Catégorie</label>
              <select id="emailp-cat-gorie" name="emailp-cat-gorie" value={form.categorie} onChange={e => setForm(f => ({ ...f, categorie: e.target.value }))} style={S.select}>
                {CATEGORIES.map(c => <option key={c.id} value={c.id}>{c.ico} {c.label}</option>)}
              </select>
            </div>
          </div>
          <div style={S.field}>
            <label htmlFor="emailp-objet-2" style={S.label}>Objet</label>
            <input id="emailp-objet-2" name="emailp-objet-2" value={form.objet} onChange={e => setForm(f => ({ ...f, objet: e.target.value }))} style={S.input} />
          </div>
          <div style={S.field}>
            <label htmlFor="emailp-corps-du-message" style={S.label}>Corps du message</label>
            <textarea id="emailp-corps-du-message" name="emailp-corps-du-message" value={form.corps} onChange={e => setForm(f => ({ ...f, corps: e.target.value }))}
    ...
    <div style={S.page}>
      <div style={S.header}>
        ...
        {isSuperAdmin && ecoles.length > 0 && (
          <>
            <label htmlFor="emailp-ecole" style={SR_ONLY}>École</label>
            <select id="emailp-ecole" name="ecole" value={ecoleId} onChange={e => setEcoleId(e.target.value)} style={{ padding: '7px 10px', borderRadius: 8, border: '1px solid #e2e8f0', fontSize: 13, fontFamily: 'inherit', outline: 'none' }}>
