...
function decisionBg(d: string | null): { bg: string; color: string } {
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          /* En-tête */
          .header { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 3px solid #1e3a5f; padding-bottom: 10px; margin-bottom: 12px; }
          .logo-zone { display: flex; flex-direction: column; }
          .ecole-nom { font-size: 13pt; font-weight: 700; color: #1e3a5f; }
          .ecole-sub { font-size: 8pt; color: #64748b; margin-top: 2px; }
          .doc-title { text-align: right; }
          .doc-title h1 { font-size: 14pt; font-weight: 700; color: #1e3a5f; }
          .doc-title p { font-size: 8pt; color: #64748b; margin-top: 2px; }

          /* Identité étudiant */
          .etudiant-card { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 10px 14px; margin-bottom: 12px; display: grid; grid-template-columns: 1fr 1fr; gap: 6px 20px; }
          .field-label { font-size: 7.5pt; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.04em; }
          .field-value { font-size: 10pt; font-weight: 600; color: #1e293b; margin-top: 1px; }
          .matricule-badge { font-family: monospace; font-size: 9.5pt; background: #e0f2fe; color: #0369a1; padding: 2px 8px; border-radius: 4px; display: inline-block; }

          /* Tableau UEs */
          .section-title { font-size: 9pt; font-weight: 700; color: #1e3a5f; text-transform: uppercase; letter-spacing: 0.05em; margin: 10px 0 6px; padding-bottom: 3px; border-bottom: 1px solid #e2e8f0; }
          table { width: 100%; border-collapse: collapse; margin-bottom: 10px; }
          th { background: #1e3a5f; color: #fff; font-size: 8pt; font-weight: 600; padding: 6px 8px; text-align: left; }
          th.center { text-align: center; }
          td { padding: 6px 8px; font-size: 9pt; border-bottom: 1px solid #f1f5f9; vertical-align: middle; }
          td.center { text-align: center; }
          tr:nth-child(even) td { background: #f8fafc; }
          .ue-code { font-family: monospace; font-size: 8pt; color: #64748b; }
          .ue-intitule { font-weight: 600; }
          .ue-type { font-size: 7.5pt; color: #94a3b8; }
          ...
          .recap-item { background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 6px; padding: 8px; text-align: center; }
          .recap-label { font-size: 7.5pt; color: #64748b; text-transform: uppercase; letter-spacing: 0.04em; }
          ...
          /* Pied */
          .footer { margin-top: 16px; padding-top: 10px; border-top: 1px solid #e2e8f0; display: flex; justify-content: space-between; align-items: flex-end; }
          .footer-mention { font-size: 8pt; color: #94a3b8; }
          .signature-zone { text-align: center; }
          .signature-line { width: 100px; border-top: 1px solid #1e293b; margin: 30px auto 4px; }
          .signature-label { font-size: 7.5pt; color: #64748b; }
          .watermark { font-size: 7pt; color: #94a3b8; text-align: center; margin-top: 10px; }
        </style>
      </head>
      <body>
        ${printContent}
      ...
      <div style={S.modal}>
        ...
        <div style={S.body}>
          <div ref={printRef} className="releve-wrap" style={{ fontFamily: 'Arial, sans-serif', fontSize: 11, color: '#1e293b', maxWidth: 750, margin: '0 auto' }}>
            ...
            {/* ── En-tête ── */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '3px solid #1e3a5f', paddingBottom: 12, marginBottom: 14 }}>
              <div>
                <div style={{ fontSize: 15, fontWeight: 800, color: '#1e3a5f' }}>{ecole.nom}</div>
                <div style={{ fontSize: 9, color: '#64748b', marginTop: 2 }}>
                  Enseignement Supérieur · Système LMD-CAMES
                </div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 15, fontWeight: 800, color: '#1e3a5f', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                  Relevé de Notes
                </div>
                <div style={{ fontSize: 9, color: '#64748b', marginTop: 2 }}>
            ...
            {/* ── Identité étudiant ── */}
            <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 6, padding: '10px 14px', marginBottom: 14, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px 24px' }}>
              <div>
                <div style={{ fontSize: 8, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Nom & Prénom</div>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#1e293b', marginTop: 1 }}>{etudiant.nom} {etudiant.prenom}</div>
              </div>
              <div>
                <div style={{ fontSize: 8, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Matricule</div>
                <div style={{ fontFamily: 'monospace', fontSize: 11, background: '#e0f2fe', color: '#0369a1', padding: '2px 8px', borderRadius: 4, display: 'inline-block', marginTop: 2 }}>
                  {etudiant.matricule}
                </div>
              </div>
              <div>
                <div style={{ fontSize: 8, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Programme</div>
                <div style={{ fontSize: 10, fontWeight: 600, color: '#1e293b', marginTop: 1 }}>{snapshot.programme_intitule}</div>
              </div>
              <div>
                <div style={{ fontSize: 8, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>Niveau · Semestre</div>
                <div style={{ fontSize: 10, fontWeight: 600, color: '#1e293b', marginTop: 1 }}>
            ...
            {/* ── Tableau UEs ── */}
            <div style={{ fontSize: 9, fontWeight: 700, color: '#1e3a5f', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 6, paddingBottom: 3, borderBottom: '1px solid #e2e8f0' }}>
              Résultats par Unité d'Enseignement
            </div>
            <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 14 }}>
              <thead>
                <tr>
                  <th style={{ background: '#1e3a5f', color: '#fff', fontSize: 8, fontWeight: 600, padding: '6px 8px', textAlign: 'left' }}>Code</th>
                  <th style={{ background: '#1e3a5f', color: '#fff', fontSize: 8, fontWeight: 600, padding: '6px 8px', textAlign: 'left' }}>Unité d'Enseignement</th>
                  <th style={{ background: '#1e3a5f', color: '#fff', fontSize: 8, fontWeight: 600, padding: '6px 8px', textAlign: 'center' }}>Crédits</th>
                  <th style={{ background: '#1e3a5f', color: '#fff', fontSize: 8, fontWeight: 600, padding: '6px 8px', textAlign: 'center' }}>Moyenne</th>
                  <th style={{ background: '#1e3a5f', color: '#fff', fontSize: 8, fontWeight: 600, padding: '6px 8px', textAlign: 'center' }}>Mention</th>
                  <th style={{ background: '#1e3a5f', color: '#fff', fontSize: 8, fontWeight: 600, padding: '6px 8px', textAlign: 'center' }}>Statut</th>
                </tr>
              </thead>
              <tbody>
                {snapshot.resultats_ue.map((ue, i) => {
                  ...
                  return (
                    <tr key={ue.ue_id}>
                      <td style={{ padding: '6px 8px', background: bg, borderBottom: '1px solid #f1f5f9', fontFamily: 'monospace', fontSize: 9, color: '#64748b' }}>
                        {ue.ue_code}
                      </td>
                      <td style={{ padding: '6px 8px', background: bg, borderBottom: '1px solid #f1f5f9' }}>
                        <div style={{ fontSize: 10, fontWeight: 600, color: '#1e293b' }}>{ue.ue_intitule}</div>
                      ...
                      <td style={{ padding: '6px 8px', background: bg, borderBottom: '1px solid #f1f5f9', textAlign: 'center', fontSize: 10 }}>
                        <span style={{ fontWeight: 600, color: '#1e293b' }}>{ue.credits_acquis}</span>
                        <span style={{ color: '#94a3b8', fontSize: 8 }}>/{ue.ue_credits}</span>
                      </td>
                      <td style={{ padding: '6px 8px', background: bg, borderBottom: '1px solid #f1f5f9', textAlign: 'center' }}>
                        {ue.est_exclu ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 8, marginBottom: 12 }}>
              {[
                ...
                <div key={label} style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 6, padding: '8px 10px', textAlign: 'center' }}>
                  <div style={{ fontSize: 7.5, fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.04em' }}>{label}</div>
            ...
            {/* ── Pied de page ── */}
            <div style={{ marginTop: 16, paddingTop: 10, borderTop: '1px solid #e2e8f0', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
              <div>
              ...
              <div style={{ textAlign: 'center' }}>
                ...
                <div style={{ fontSize: 8, color: '#64748b' }}>Le Chef d'Établissement</div>
              </div>
            </div>
