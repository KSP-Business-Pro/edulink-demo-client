...
import type { LigneDelib } from '../../types/deliberations.types';
...
interface ReleveNote {
  etudiant_id: string; mention: string | null; credits_valides: number;
  decision: string | null; verrouille: boolean;
}
...
export default function RelevesPage() {
  ...
  const loadReleves = useCallback(async () => {
    ...
    try {
      const [insRes, relevesRes, blocRes] = await Promise.all([
        ...
        supabase.from('releves_notes').select('*').eq('semestre_id', semId),
        supabase.from('regles_ecole')
          .select('blocage_releve_impaye,tolerance_impaye_releve')
          .eq('ecole_id', ecoleId).maybeSingle(),
      ]);

      const etus = ((insRes.data ?? []).map((i: any) => i.etudiants).filter(Boolean) as ReleveEtudiant[])
        .sort((a, b) => a.nom.localeCompare(b.nom));
      setEtudiants(etus);

      const rMap: Record<string, ReleveNote> = {};
      (relevesRes.data ?? []).forEach((r: any) => { rMap[r.etudiant_id] = r; });
      setRelevesMap(rMap);

      // Blocage impayé
      const bloc: BlocageRegles = blocRes.data ?? { blocage_releve_impaye: false, tolerance_impaye_releve: 0 };
      ...
      if (bloc.blocage_releve_impaye) {
        ...
        if (ids.length) {
          ...
          (facs ?? []).forEach((f: any) => {
            ...
            sMap[f.etudiant_id] = (sMap[f.etudiant_id] || 0) + du;
          });
          setSoldesMap(sMap);
        }
      } else {
  ...
  async function renvoyerEmail(etudiant: ReleveEtudiant) {
    ...
    } catch (e: any) { showToast('Erreur réseau', 'error'); }
  }
  ...
  async function handleRepublierTous() {
    ...
    if (!confirm(`Recalculer et republier ${publies.length} relevé(s) déjà publiés ?\nAucun email ne sera envoyé.`)) return;
  ...
  async function handleVerrou(etudiantId: string, isLocked: boolean) {
    ...
    const msg  = isLocked ? 'Déverrouiller ce relevé ?' : 'Verrouiller ce relevé ? Il deviendra définitif.';
  ...
  const nbBloques = soldesMap
    ? etudiants.filter(et => !relevesMap[et.id] && (soldesMap[et.id] || 0) > tolerance).length
    : 0;
    ...
    <div style={{ padding: '1.5rem', paddingBottom: '2rem' }}>
      ...
      <div className="top">
        <div>
          ...
          <div className="page-subtitle">Publication · verrouillage · envoi email</div>
        </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1rem', padding: '.85rem 1rem', background: '#f9fafb', borderRadius: 10, border: '1px solid #e5e7eb', flexWrap: 'wrap' }}>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#374151', cursor: 'pointer', textTransform: 'none', letterSpacing: 0, fontWeight: 400 }}>
              <input type="checkbox" checked={sendEmail} onChange={e => setSendEmail(e.target.checked)} style={{ width: 14, height: 14, margin: 0 }} />
              📧 Email à la publication
            </label>

            {/* Recherche */}
            <input type="search" value={search} onChange={e => setSearch(e.target.value)}
              placeholder="🔍 Rechercher…" style={{ padding: '6px 10px', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 12.5, fontFamily: 'inherit', width: 160 }} />
            ...
            <button onClick={handleRepublierTous} disabled={!!pubProgress}
              ...
              title="Recalculer et republier tous les relevés déjà publiés">
              🔄 Republier tous
            </button>
          </div>
