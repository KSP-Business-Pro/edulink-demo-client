...
// Auto-génère un matricule si absent : ECO-NOM3-RAND4
...
export async function importEtudiants(
  ...
  // Charger les matricules existants pour détecter les mises à jour
  // supabase déjà importé en haut du module
  ...
  for (let i = 0; i < rows.length; i += BATCH_SIZE) {
    ...
    batch.forEach((row, bIdx) => {
      const lineNum = i + bIdx + 2; // numéro ligne Excel (1-indexed + header)
      try {
    ...
    if (toInsert.length) {
      ...
      if (error) {
        result.errors.push(`Batch insert [${i + 1}-${i + toInsert.length}] : ${error.message}`);
      } else {
    ...
    for (const row of toUpdate) {
      ...
      if (error) {
        result.errors.push(`Mise à jour ${row.matricule} : ${error.message}`);
      } else {
        result.updated++;
      }
    }
