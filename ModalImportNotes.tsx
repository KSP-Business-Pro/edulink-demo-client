    ...
    <div style={S.overlay} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={S.modal}>
        ...
        <div style={S.body}>
          {/* Résultat import */}
          {imported && (
            <div role="status" style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 10, padding: '1rem' }}>
          ...
          {!imported && (
            <>
              ...
              <div style={S.field}>
                <label style={S.label} htmlFor="import-eval">Évaluation cible <span style={{ color: '#dc2626' }}>*</span></label>
                <select id="import-eval" name="evaluation" style={S.input} value={evalId} onChange={e => setEvalId(e.target.value)}>
                  {evaluations.map(e => (
              ...
              {/* Upload fichier */}
              <div style={S.field}>
                <label style={S.label} htmlFor="import-fichier">Fichier CSV ou Excel <span style={{ color: '#dc2626' }}>*</span></label>
                <input ref={fileRef} id="import-fichier" name="fichier" type="file" accept=".csv,.xlsx,.xls"
                  onChange={handleFile}
                  aria-invalid={!!parseErr}
                  aria-describedby={parseErr ? 'import-fichier-error' : undefined}
              ...
              {/* Erreur */}
              {parseErr && (
                <div id="import-fichier-error" role="alert" style={{ background: '#fef2f2', color: '#991b1b', padding: '8px 12px', borderRadius: 8, fontSize: 12, border: '1px solid #fecaca' }}>
                  {parseErr}
                </div>
              )}
