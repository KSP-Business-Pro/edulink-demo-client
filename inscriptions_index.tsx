...
export default function InscriptionsPage() {
  ...
  return (
    <div style={S.page}>
      ...
      {/* ── Filtres ── */}
      <div style={S.filters}>
        <label htmlFor="insc-filter-search" style={S.srOnly}>Rechercher par nom, prénom ou matricule</label>
        <input
          type="text"
          id="insc-filter-search"
          name="search"
        ...
        />
        <label htmlFor="insc-filter-semestre" style={S.srOnly}>Filtrer par semestre</label>
        <select id="insc-filter-semestre" name="filterSemestre" style={S.filterInput} value={filterSem} onChange={e => setFilterSem(e.target.value)}>
          <option value="">Tous les semestres</option>
          {semestres.map(s => <option key={s.id} value={s.id}>{s.libelle}</option>)}
        </select>
        <label htmlFor="insc-filter-promotion" style={S.srOnly}>Filtrer par promotion</label>
        <select id="insc-filter-promotion" name="filterPromotion" style={S.filterInput} value={filterPromo} onChange={e => setFilterPromo(e.target.value)}>
          <option value="">Toutes les promotions</option>
          {promotions.map(p => <option key={p.id} value={p.id}>{p.nom}</option>)}
        </select>
        <label htmlFor="insc-filter-statut" style={S.srOnly}>Filtrer par statut</label>
        <select id="insc-filter-statut" name="filterStatut" style={{ ...S.filterInput, maxWidth: 150 }} value={filterStatut} onChange={e => setFilterStatut(e.target.value)}>
                    ...
                    <tr key={ins.inscription_id} style={S.tr}>
                      ...
                      <td style={S.td}>
                        <select
                          ...
                          aria-label={`Statut de l'inscription de ${ins.etudiant_nom} ${ins.etudiant_prenom ?? ''}`}
                          style={{
                      ...
                      <td style={{ ...S.td }}>
                        <button
                          ...
                          aria-label={`Supprimer l'inscription de ${ins.etudiant_nom} ${ins.etudiant_prenom ?? ''}`}
                        >
                          🗑️
                        </button>
                      </td>
...
const S = {
  ...
  srOnly:      { position: 'absolute' as const, width: 1, height: 1, overflow: 'hidden', clip: 'rect(0 0 0 0)', whiteSpace: 'nowrap' as const } as React.CSSProperties,
