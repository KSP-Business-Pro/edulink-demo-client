...
export default function EnseignantsPage() {
  ...
  function openEdit(e: Enseignant) {
    setEditId(e.id);
    setForm({ nom: e.nom, prenom: e.prenom ?? '', grade: e.grade ?? GRADES[5], specialite: e.specialite ?? '', email: e.email ?? '', telephone: e.telephone ?? '', statut: e.statut });
    setFormError('');
    setModalOpen(true);
  }
  ...
  async function handleSubmit(ev: React.FormEvent) {
    ...
    try {
      const payload = { ecole_id: ecoleId, nom: form.nom.trim().toUpperCase(), prenom: form.prenom.trim() || null, grade: form.grade || null, specialite: form.specialite.trim() || null, email: form.email.trim() || null, telephone: form.telephone.trim() || null, statut: form.statut };
      if (editId) {
  ...
  async function openMatieres(e: Enseignant) {
    ...
    setMatieres((data ?? []) as MatiereLien[]);
  ...
  async function handleImport() {
    ...
    for (const row of importRows) {
      const gradeVal = GRADES.find(g => g.toLowerCase().includes((row.grade ?? '').toLowerCase())) ?? 'Vacataire';
    ...
    setImporting(false);
    showToast(`${ok} enseignant(s) importé(s)${skip ? ` · ${skip} erreur(s)` : ''}`, skip ? 'info' : 'success');
