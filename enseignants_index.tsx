...
const enseignantColumns: RTColumn<Enseignant>[] = [
  {
    key: 'nom',
    label: 'Nom',
    primary: true,
  ...
  },
  {
    key: 'specialite',
    label: 'Spécialité',
    render: e => <span style={{ fontSize: 12, color: '#6b7280' }}>{e.specialite ?? '—'}</span>,
  },
  {
    key: 'grade',
    label: 'Grade',
  ...
  },
  {
    key: 'telephone',
    label: 'Téléphone',
    render: e => <span style={{ fontSize: 12, color: '#6b7280' }}>{e.telephone ?? '—'}</span>,
  },
  {
    key: 'statut',
    label: 'Statut',
    render: e => <span className={`badge ${e.statut === 'actif' ? 'green' : 'gray'}`}>{e.statut}</span>,
  },
];
      ...
      <div className="top">
        ...
        <div className="top-actions">
          {isSuperAdmin && ecoles.length > 0 && (
            <>
              <label htmlFor="enseignants-ecole" style={SR_ONLY}>École</label>
              <select id="enseignants-ecole" name="ecole" value={ecoleId} onChange={e => setEcoleId(e.target.value)}
                style={{ padding: '7px 12px', border: '1px solid #e5e7eb', borderRadius: 8, fontSize: 13, fontFamily: 'inherit' }}>
                {ecoles.map(e => <option key={e.id} value={e.id}>{e.nom}</option>)}
              </select>
            </>
          )}
          <label htmlFor="enseignants-search" style={SR_ONLY}>Rechercher un enseignant</label>
          <input type="search" id="enseignants-search" name="search" autoComplete="off" value={search} onChange={e => setSearch(e.target.value)}
          ...
          <div className="modal" style={{ width: 500, padding: '1.5rem' }}>
            ...
            <form onSubmit={handleSubmit} autoComplete="off">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.75rem', marginBottom: '.85rem' }}>
                <div>
                  <label htmlFor="ens-grade">Grade *</label>
                  <select id="ens-grade" name="grade" value={form.grade}
                ...
                <div>
                  ...
                  <select id="ens-statut" name="statut" value={form.statut}
              ...
              <div style={{ marginBottom: '.85rem' }}>
                <label htmlFor="ens-specialite">Spécialité</label>
                <input id="ens-specialite" name="specialite" type="text" value={form.specialite}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '.75rem', marginBottom: '1.2rem' }}>
                <div>
                  ...
                  <input id="ens-email" name="email" type="email" value={form.email}
                    onChange={e => setForm(f => ({ ...f, email: e.target.value }))}
                    style={{ width: '100%', marginTop: 4 }} placeholder="enseignant@email.com" />
                </div>
                <div>
                  <label htmlFor="ens-telephone">Téléphone</label>
                  <input id="ens-telephone" name="telephone" type="tel" value={form.telephone}
      ...
      {showImport && (
        <div className="modal-overlay open" onClick={e => e.target === e.currentTarget && setShowImport(false)}>
          <div className="modal" style={{ width: 560, padding: '1.5rem' }}>
            ...
            <div style={{ marginBottom: '.85rem' }}>
              <label htmlFor="ens-import-fichier">Fichier CSV ou Excel *</label>
              <input ref={importFileRef} id="ens-import-fichier" name="fichier" type="file" accept=".csv,.xlsx,.xls" onChange={handleImportFile}
                style={{ width: '100%', marginTop: 4 }} />
            </div>
            {importParseErr && (
              <div role="alert" style={{ background: '#fee2e2', color: '#dc2626', padding: '8px 12px', borderRadius: 8, fontSize: 12, marginBottom: '.85rem' }}>
                {importParseErr}
              </div>
            )}
            {importRows && (
